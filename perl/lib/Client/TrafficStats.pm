...
Client::TrafficStats - Write daily web traffic stats as CSV, XML, HTML or JSON

=head1 VERSION

This document refers to version 1.0 of Client::TrafficStats, released Jul 07, 2015

=head1 DESCRIPTION

Client::TrafficStats writes daily web traffic stats as CSV, XML, HTML and JSON.
...
Create a new Client::TrafficStats object
    ...
    foreach my $channel_id (@channel_ids)
    {
        ...
        foreach my $date (@dates)
        {
            ...
            for ($date = $days_from; $date <= $days_to; $date++)
            {
                ...
                $self->get_traffic_stats( traffic => $traffic,
                                          request => $request,
                                          date => $date,
...
sub get_traffic_stats
{
    ...
    if ($date < 20000000)
    {
        $time = time() - 86400 * $date; # $date is a number of days
        $date = Utils::Time->get_date($time, $site->{time_zone});
    }
    $time = Utils::Time->get_time($date, '00:00:00', $site->{time_zone});
    my $secs = time() - $time;
    ...
    my $traffic_day = $channel->{traffic}[$self->{traffic_days}++] = { date_requested => $self->{date_requested}, date => $date, time => $time, secs => $secs, visit => [] };
    ...
    my $query = $data_server->sql("select *, V.visit_id, V.campaign from $database.Visit V join $database.TrafficStats TS on TS.visit_id = V.visit_id where TS.time between ? and ? and TS.channel_id = $channel_id order by TS.time desc$limit", $time, $time + Utils::Time::DAY_SECS);
